...
function formatHistoryItemWarrantyHtml(entry) {
  if (!entry.warrantyMonths || !entry.warrantyExpiresAt) {
    return '';
  }

  const label = `Warranty: ${entry.warrantyMonths} mo · valid until ${formatWarrantyExpiryDate(entry.warrantyExpiresAt)}`;
...
function buildReceiptHtml({ shop, history }) {
  ...
  const displayOrderId =
    history.cartNumber != null && history.cartNumber > 0
      ? `#${history.cartNumber}`
      : history.orderId?.trim() || '—';
    ...
    .map((entry) => {
      ...
      const unitLine =
        entry.unitCost != null
          ...
          : '';
      const warrantyLine = formatHistoryItemWarrantyHtml(entry);
