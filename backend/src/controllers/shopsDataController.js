...
const { buildIndustryOnboardingFields } = require('../utils/industryHelper');
...
const { buildAdditionalPayments } = require('../services/billingCheckService');
const {
  formatShopForLogin,
  formatUserForLogin,
...
const createShopOnboarding = async (req, res) => {
  try {
    ...
    const shop = await ShopsData.create({
      ...
      ...buildIndustryOnboardingFields(industryType),
    });
...
const SHOP_USERS_FEATURE_FIELDS =
  'shopId maxUsers isAdditionalUsersAdded numAdditionalUsers nextPaymentDate subscriptionType';
...
const setSubscription = async (req, res) => {
  try {
    ...
    const paymentAmount = baseFee + additionalTotal;
...
const removeOnboardingData = async (req, res) => {
  try {
    ...
    const { deleted, cronReportsScrubbed } = await deleteAllShopScopedData(normalizedShopId);

    res.status(200).json({
      success: true,
      shopId: normalizedShopId,
      removedUsers: deleted.users,
      deleted,
      cronReportsScrubbed,
