...
const REVIEWABLE_STATUSES = ["pending", "notPaid"];
...
function isReviewablePaymentStatus(status) {
  return REVIEWABLE_STATUSES.includes(status);
}

function reviewableStatusErrorMessage(action) {
  return `Only payments with status "${REVIEWABLE_STATUSES.join('" or "')}" can be ${action}`;
}

function ensureSubmittedDateForApproval(payment) {
...
function mapSubscriptionPaymentListItem(payment, shop, req) {
  ...
  return {
    ...
    shopSubscriptionType: shop?.subscriptionType ?? null,
    receiptNumber: formatted.receiptNumber,
    receiptImagePath: formatted.receiptImagePath,
    receiptImageUrl: formatted.receiptImageUrl,
    receiptImageAvailable: formatted.receiptImageAvailable,
...
const listPendingPayments = async (req, res) => {
  try {
    ...
    const query = { status: { $in: REVIEWABLE_STATUSES } };
...
const getSubscriptionPayments = async (req, res) => {
  try {
    ...
    const shops = await ShopsData.find({ shopId: { $in: shopIds } })
      .select(
        "shopId shopName ownerFirstName ownerLastName shopMobileNumber email status subscriptionDueDays subscriptionType isSubscriptionChangePending",
      )
      .lean();
    const shopById = new Map(shops.map((s) => [s.shopId, s]));
 ...
 * set status to changeSubscription and force logout all shop users.
 * Other shop fields (subscriptionType, nextPaymentDate, etc.) remain from the approval step.
 */
async function applySubscriptionChangePendingAfterApprove(shop) {
  ...
  shop.status = "changeSubscription";
...
const rejectUpfrontPayment = async (req, res) => {
  try {
    ...
    if (!isReviewablePaymentStatus(payment.status)) {
      return res.status(400).json({
        success: false,
        message: reviewableStatusErrorMessage("rejected"),
...
const approveUpfrontPayment = async (req, res) => {
  try {
    ...
    if (!isReviewablePaymentStatus(payment.status)) {
      return res.status(400).json({
        success: false,
        message: reviewableStatusErrorMessage("approved"),
        currentStatus: payment.status,
      });
    }
    ...
    ensureSubmittedDateForApproval(payment);
...
/**
 * Dashboard: generate or refresh an upfront payment confirmation for a shop.
 * Updates shop customer details + amount, then creates/updates a notPaid upFront invoice.
 */
const generateUpfrontPaymentConfirmation = async (req, res) => {
  try {
    const body = req.body ?? {};
    const errors = [];

    const shopId = normalizeRequiredText(body.shopId, "shopId", errors);
    ...
    if (body.paymentAmount === undefined || body.paymentAmount === null || body.paymentAmount === "") {
    ...
    const description =
      body.description != null && String(body.description).trim() !== ""
        ...
        : "One-time upfront payment for Smart Cost setup";
    ...
    if (shop.isOneTimePaymentDone) {
      return res.status(400).json({
        success: false,
        message: "Upfront payment is already completed for this shop",
      });
    }
    ...
    if (existingPayment?.status === "approve") {
      return res.status(400).json({
        success: false,
        message: "Upfront payment is already approved for this shop",
      });
    }
    ...
    if (existingPayment?.status === "pending") {
      return res.status(400).json({
        success: false,
        message:
          "Customer has already submitted a receipt for this upfront payment. Review it under Onboarding payments.",
      });
    }

    shop.shopName = shopName;
    shop.address = address;
    shop.shopMobileNumber = shopMobileNumber;
    shop.oneTimePaymentAmount = paymentAmount;

    let payment = existingPayment;
    let created = false;

    if (payment && payment.status === "notPaid") {
      ...
      payment.reason = null;
      payment.submittedDate = null;
      payment.receiptImagePath = UPFRONT_INVOICE_IMAGE_PLACEHOLDER;
      await payment.save();
    } else if (!payment) {
    ...
    res.status(created ? 201 : 200).json({
      success: true,
      message: created
        ? "Upfront payment confirmation generated"
        : "Upfront payment confirmation updated",
      payment: formatPaymentRecord(payment.toObject(), req),
      shop: formatShopSummary(shop.toObject()),
    });
  } catch (error) {
    ...
    console.log("error in generateUpfrontPaymentConfirmation", error);
    res.status(500).json({ success: false, message: error.message });
  }
};
...
module.exports = {
  ...
  generateUpfrontPaymentConfirmation,
