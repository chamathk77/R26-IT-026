...
function buildProductFormData(
  ...
  appendOptionalField(formData, 'productNumber', payload.productNumber?.trim() || null);
  if (payload.isInventoryAvailable === true && hasNumericValue(payload.qty)) {
    formData.append('qty', String(payload.qty));
  }
  if (payload.warrantyAvailable != null) {
    formData.append('warrantyAvailable', String(payload.warrantyAvailable));
  }
  if (payload.warrantyAvailable === true && hasNumericValue(payload.warrantyMonths)) {
...
function buildProductJsonBody(payload: ProductBodyFields) {
  ...
  if (payload.barcode !== undefined) {
    const trimmed = payload.barcode == null ? null : String(payload.barcode).trim();
    if (trimmed) body.barcode = trimmed;
    else body.barcode = null;
  }
  if (payload.productNumber !== undefined) {
    const trimmed =
      payload.productNumber == null ? null : String(payload.productNumber).trim();
    if (trimmed) body.productNumber = trimmed;
    else body.productNumber = null;
  }
  if (payload.isInventoryAvailable === true && hasNumericValue(payload.qty)) {
    body.qty = payload.qty;
  }
  if (payload.warrantyAvailable != null) {
    body.warrantyAvailable = payload.warrantyAvailable;
  }
  if (payload.warrantyAvailable === true && hasNumericValue(payload.warrantyMonths)) {
    body.warrantyMonths = payload.warrantyMonths;
  } else if (payload.warrantyAvailable === false) {
...
export const fetchProducts_Service = createAsyncThunk(
  'product/fetchAll',
  async (params: FetchProductsParams | void, { rejectWithValue }) => {
    try {
      await ensureInternetConnection();

      const query: Record<string, string> = {};
      if (params?.productNumber?.trim()) {
        query.productNumber = params.productNumber.trim();
      }
