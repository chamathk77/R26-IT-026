...
export const createCartSession_Service = createAsyncThunk(
  'cart/createSession',
  async (payload: CreateCartSessionRequest | void, { rejectWithValue }) => {
    try {
      await ensureInternetConnection();
...
export const createNewPendingCart_Service = createAsyncThunk(
  ...
  async (payload: CreateCartSessionRequest | void, { dispatch, rejectWithValue }) => {
    try {
      const session = await dispatch(createCartSession_Service(payload ?? undefined)).unwrap();
...
export const addProductToPendingCart_Service = createAsyncThunk(
  ...
  async (
    payload: {
      ...
      orderMeta?: CreateCartSessionRequest;
    },
    ...
    try {
      ...
      let orderType = payload.forceNewCart ? null : state.CartReducer.activeSession.orderType ?? null;
      let tableId = payload.forceNewCart ? null : state.CartReducer.activeSession.tableId ?? null;
      let orderLabel = payload.forceNewCart ? '' : state.CartReducer.activeSession.orderLabel ?? '';

      if (!sessionId) {
        const session = await dispatch(createCartSession_Service(payload.orderMeta)).unwrap();
...
export const sendCartToKitchen_Service = createAsyncThunk(
  'cart/sendToKitchen',
  async (sessionId: string, { dispatch, rejectWithValue }) => {
    try {
      await ensureInternetConnection();

      const response = await apiClient.post<SendCartToKitchenResponse>(
        `/api/cart/sessions/${sessionId}/send-to-kitchen`,
      );

      if (isHttpSuccess(response.status) && response.data?.success && response.data.kitchenTicket) {
        await dispatch(fetchPendingCartSessions_Service());
        return response.data;
      }
      ...
      const apiError: ApiErrorResponse = {
        error: 'Error',
        message:
          (response.data as SendCartToKitchenResponse & { message?: string })?.message ||
...
export const proceedCartSession_Service = createAsyncThunk(
  ...
  async (sessionId: string, { dispatch, getState, rejectWithValue }) => {
    try {
      ...
      const pendingSessions = (getState() as RootState).CartReducer.pendingSessions.items;
      const pendingSession = pendingSessions.find((session) => session.sessionId === sessionId);
      ...
      if (isHttpSuccess(response.status) && response.data?.success) {
        ...
        return {
          ...response.data,
          cartNumber,
          orderType: pendingSession?.orderType ?? null,
          orderLabel: pendingSession?.orderLabel ?? '',
        };
      }
...
export async function fetchCartSessionDetail_Service(sessionId: string) {
  await ensureInternetConnection();

  const response = await apiClient.get<CartSessionDetailResponse>(
    `/api/cart/sessions/${sessionId}/detail`,
  );
...
export async function changeCartSessionTable_Service(sessionId: string, tableId: string) {
  await ensureInternetConnection();

  const response = await apiClient.patch<ChangeCartTableResponse>(
    `/api/cart/sessions/${sessionId}/table`,
    { tableId },
  );
