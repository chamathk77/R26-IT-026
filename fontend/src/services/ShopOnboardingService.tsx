...
export const selectNewSubscription_Service = createAsyncThunk(
  "shopOnboarding/selectNewSubscription",
  async (payload: SelectNewSubscriptionRequest, { rejectWithValue }) => {
    try {
      await ensureInternetConnection();

      const response = await apiClient.post<SelectNewSubscriptionResponse>(
        "/api/shops/subscription/change/select",
        payload,
      );

      if (isHttpSuccess(response.status)) {
        console.log("Select new subscription response:", response.data);
        return response.data;
      }

      const apiError: ApiErrorResponse = {
        error: "Error",
        message: "Subscription plan selection failed",
        status: response.status,
        timestamp: new Date().toISOString(),
      };
      return rejectWithValue(apiError);
