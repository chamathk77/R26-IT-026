...
import { CartOrder, CartSessionSummary, CreateCartSessionResponse, CartOrderType } from '../../type/cart';
...
export const CartSlice = createSlice({
  ...
  reducers: {
    ...
    setCartTabSelection: (
      ...
      state.cartTab.orderType = action.payload.orderType ?? null;
      state.cartTab.orderLabel = action.payload.orderLabel ?? '';
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchAddedCartSessions_Service.fulfilled, (state, action) => {
      ...
      if (state.cartTab.sessionId) {
        ...
        if (!stillAdded) {
          ...
          const match = state.addedSessions.items.find(
            (session) => session.sessionId === state.cartTab.sessionId,
          );
          if (match) {
            state.cartTab.orderType = match.orderType ?? null;
            state.cartTab.orderLabel = match.orderLabel ?? '';
            if (match.cartNumber != null) {
              state.cartTab.cartNumber = match.cartNumber;
            }
          }
        }
      }
    ...
    builder.addCase(addProductToPendingCart_Service.fulfilled, (state, action) => {
      ...
      state.activeSession = mapToActiveSession({
        sessionId: action.payload.sessionId,
        cartNumber: action.payload.cartNumber,
        orderType: action.payload.orderType,
        tableId: action.payload.tableId,
        orderLabel: action.payload.orderLabel,
      });
    });
    ...
    builder.addCase(createCartSession_Service.fulfilled, (state, action) => {
      state.addToCart.loading = false;
      state.addToCart.error = null;
      state.activeSession = mapToActiveSession(action.payload);
    });
