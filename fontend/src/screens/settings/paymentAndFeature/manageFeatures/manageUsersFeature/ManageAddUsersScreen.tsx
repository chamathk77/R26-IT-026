...
import {
  ...
  calculateAdditionalUsersCharge,
  DEFAULT_MAX_USERS,
  formatLkr,
  formatSubscriptionBillingPeriod,
  getSubscriptionBillingMonths,
...
function buildUsersUpdateSuccessMessage(
  features: ShopUsersFeaturesPayload,
  subscriptionType?: string | null,
): string {
  const { isAdditionalUsersAdded, numAdditionalUsers, maxUsers } = features;
  const planType = subscriptionType ?? features.subscriptionType ?? null;

  if (isAdditionalUsersAdded && numAdditionalUsers != null && numAdditionalUsers > 0) {
    const charge = calculateAdditionalUsersCharge(planType, numAdditionalUsers);
    const billingMonths = getSubscriptionBillingMonths(planType);
    const userWord = numAdditionalUsers === 1 ? 'user' : 'users';
    const periodLabel =
      billingMonths === 1
        ? 'your next subscription payment'
        : `your next ${formatSubscriptionBillingPeriod(planType)} subscription payment`;
    return `Your account now supports ${maxUsers} users. The charge for ${numAdditionalUsers} additional ${userWord} (${formatLkr(charge)} for ${billingMonths} month${billingMonths === 1 ? '' : 's'}) will be added to ${periodLabel}.`;
  }

  return `Your account now supports ${maxUsers} users. No additional user charges will be added to your next subscription payment.`;
}
...
export default function ManageAddUsersScreen({ navigation }: Props) {
  ...
  const billingMonths = useMemo(
    () => getSubscriptionBillingMonths(subscriptionType),
    [subscriptionType],
  );

  const additionalUsersCharge = useMemo(() => {
    if (!form?.isAdditionalUsersAdded || parsedAdditionalUsers < 1) {
      return 0;
    }
    return calculateAdditionalUsersCharge(subscriptionType, parsedAdditionalUsers);
  }, [form?.isAdditionalUsersAdded, parsedAdditionalUsers, subscriptionType]);
  ...
  const applyLoadedUsers = useCallback((features: ShopUsersFeaturesPayload) => {
    ...
    setSubscriptionType(features.subscriptionType ?? null);
  ...
  const onUpdate = async () => {
    ...
    try {
      ...
      setSavedSnapshot(buildSnapshot(form));
      setLoadedMaxUsers(response.features.maxUsers);

      show_Alert(
        'success',
        'Settings saved',
        buildUsersUpdateSuccessMessage(response.features, subscriptionType),
        1,
        false,
        'OK',
        () => {
        ...
        {showLoader ? (
          ...
          <>
            ...
            >
              ...
              >
                <View style={[styles.billingTopIcon, { backgroundColor: '#fef3c7' }]}>
                  <Ionicons name="calendar-outline" size={20} color="#d97706" />
                </View>
                <View style={styles.billingTopBody}>
                  <Text
                    style={[styles.billingTopLabel, { color: paperTheme.colors.onSurfaceVariant }]}
                  >
                    Next payment date
                  </Text>
                  <Text style={[styles.billingTopValue, { color: paperTheme.colors.onSurface }]}>
              ...
              >
                ...
                <View style={styles.usersHeroBody}>
                  ...
                  >
                    ...
                    {billingMonths > 1
                      ? ` (billed for ${billingMonths} months on ${formatSubscriptionBillingPeriod(subscriptionType)} plans).`
                      : '.'}
                  </Text>
                </View>
              </View>
                ...
                {form.isAdditionalUsersAdded && parsedAdditionalUsers > 0 ? (
                  <>
                    ...
                    >
                      ...
                      additional user per month, multiplied by your{' '}
                      {formatSubscriptionBillingPeriod(subscriptionType)} billing period (
                      {billingMonths} month{billingMonths === 1 ? '' : 's'}).
                    </Text>
                    <SummaryRow
                      label="Add-on total (next payment)"
                      value={formatLkr(additionalUsersCharge)}
                      paperTheme={paperTheme}
                      emphasize
                    />
                    <Text
