...
export default function PrinterConnectionScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    if (status.state === 'connecting' || scanning) {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      const saved = await getSavedPrinterConfig();
      if (!mounted) {
        return;
      }
      if (saved?.mode === 'lan') {
          ...
          >
            ...
            {mode === 'lan' ? (
              ...
              >
                ...
                >
                  Phone and printer must be on the same Wi‑Fi / LAN.
                </Text>
