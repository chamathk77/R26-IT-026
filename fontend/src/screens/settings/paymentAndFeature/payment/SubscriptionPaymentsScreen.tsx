...
function PaymentDetailRow({
  ...
  const valueColor =
    valueTone === 'warning' ? '#b45309' : paperTheme.colors.onSurface;
  ...
  return (
    <View style={paymentStyles.detailRow}>
      ...
      <Text
        style={[
          paymentStyles.detailValue,
          valueTone === 'warning' ? paymentStyles.detailValueWarning : null,
          { color: valueColor },
        ]}
      >
        {value}
...
function PaymentHistoryCard({
  ...
  const isSubmitted = hasSubmittedDate(payment.submittedDate);
  ...
  return (
    ...
    >
      ...
      <View style={paymentStyles.metaRow}>
        <View
          style={[
            paymentStyles.metaItem,
            !isSubmitted ? paymentStyles.metaItemPending : null,
            !isSubmitted
              ? {
                  backgroundColor:
                    resolvedTheme === 'dark' ? 'rgba(245, 158, 11, 0.12)' : '#fffbeb',
                  borderColor: '#f59e0b',
                }
              : null,
          ]}
        >
          <Ionicons
            name={isSubmitted ? 'calendar-outline' : 'alert-circle-outline'}
            size={14}
            color={isSubmitted ? paperTheme.colors.onSurfaceVariant : '#b45309'}
          />
          <Text
            style={[
              paymentStyles.metaText,
              !isSubmitted ? paymentStyles.metaTextPending : null,
              {
                color: isSubmitted ? paperTheme.colors.onSurfaceVariant : '#b45309',
              },
            ]}
          >
            {isSubmitted
              ? `Submitted ${formatSubmittedDate(payment.submittedDate)}`
              : 'Not submitted yet'}
          </Text>
        </View>
      ...
      {isExpanded ? (
        <View style={paymentStyles.expandedSection}>
          ...
          <PaymentDetailRow
            ...
            value={formatSubmittedDateTime(payment.submittedDate)}
            paperTheme={paperTheme}
            valueTone={isSubmitted ? 'default' : 'warning'}
          />
          <PaymentDetailRow
            label="Created"
...
export default function SubscriptionPaymentsScreen({ navigation }: Props) {
  ...
  const hasUnpaidPayments = useMemo(
    () => filteredPayments.some((payment) => shouldShowPayNow(payment.status)),
    [filteredPayments],
  );
      ...
      >
        ...
        >
            ...
            {latestPayment ? (
              <View
                style={[
                  ...
                  {
                    backgroundColor: hasSubmittedDate(latestPayment.submittedDate)
                      ? resolvedTheme === 'dark'
                        ? '#422006'
                        : '#fffbeb'
                      : resolvedTheme === 'dark'
                        ...
                        : '#fff7ed',
                    borderColor: hasSubmittedDate(latestPayment.submittedDate)
                      ? '#f59e0b'
                      : '#fb923c',
                  },
                ]}
              >
                <Text
                  style={[
                    paymentStyles.latestLabel,
                    {
                      color: hasSubmittedDate(latestPayment.submittedDate)
                        ? '#92400e'
                        : '#c2410c',
                    },
                  ]}
                >
                  {hasSubmittedDate(latestPayment.submittedDate)
                    ? 'Latest submission'
                    : isInAppBillingAllowed
                      ? 'Awaiting receipt submission'
                      : 'Awaiting payment confirmation'}
                </Text>
                <Text
                  style={[
                    paymentStyles.latestValue,
                    {
                      color: hasSubmittedDate(latestPayment.submittedDate)
                        ? '#78350f'
                        : '#9a3412',
                ...
                >
                  {hasSubmittedDate(latestPayment.submittedDate)
                    ? `Submitted ${formatSubmittedDate(latestPayment.submittedDate)}`
                    : isInAppBillingAllowed
                      ? 'Receipt not submitted yet. Use Pay now to upload your bank transfer receipt.'
                      : 'See bank details below, then tap Contact admin on the payment.'}
                </Text>
              </View>
            ) : null}
          </View>
          ...
          {!isInAppBillingAllowed && hasUnpaidPayments ? (
            <View style={paymentStyles.bankDetailsWrap}>
