    ...
    <View style={styles.skeletonList}>
      {Array.from({ length: count }, (_, index) => (
        <View
          ...
          style={[
            styles.rowCard,
            styles.skeletonCardSpacing,
            { backgroundColor: cardColor, borderColor },
          ]}
        >
          <View style={styles.rowPressable}>
            <SkeletonBone width={44} height={44} borderRadius={12} color={boneColor} opacity={pulse} />
            <View style={styles.rowBody}>
              <SkeletonBone width="72%" height={14} borderRadius={7} color={boneColor} opacity={pulse} />
              <SkeletonBone width="40%" height={11} borderRadius={6} color={boneColor} opacity={pulse} />
              <View style={styles.rowChipWrap}>
                <SkeletonBone width={58} height={22} borderRadius={999} color={boneColor} opacity={pulse} />
                <SkeletonBone width={70} height={22} borderRadius={999} color={boneColor} opacity={pulse} />
              </View>
              <SkeletonBone width="55%" height={11} borderRadius={6} color={boneColor} opacity={pulse} />
              <SkeletonBone width="35%" height={11} borderRadius={6} color={boneColor} opacity={pulse} />
            </View>
            <View style={styles.rowTrailing}>
              <SkeletonBone width={64} height={16} borderRadius={8} color={boneColor} opacity={pulse} />
...
export default function HistoryScreen({ navigation }: Props) {
  ...
  const buildFetchParams = useCallback(
    (page = 1, append = false, nextFilters = filters) => ({
      ...
      cartNumber: nextFilters.cartNumber,
  ...
  const handleApplyFilters = useCallback(() => {
    ...
    const trimmedOrderQuery = draftFilters.orderId.trim().replace(/^#/, '');
    let nextOrderId = '';
    let nextCartNumber: number | undefined;

    if (trimmedOrderQuery) {
      const parsedCartNumber = Number.parseInt(trimmedOrderQuery, 10);
      const isWholeNumber =
        Number.isInteger(parsedCartNumber) &&
        parsedCartNumber > 0 &&
        String(parsedCartNumber) === trimmedOrderQuery;

      if (isWholeNumber) {
        nextCartNumber = parsedCartNumber;
      } else if (trimmedOrderQuery.length < 6) {
        show_Alert(
          'error',
          'Invalid order number',
          'Enter an order number like 5, or the full stored order id.',
          1,
          false,
          'OK',
          () => {},
        ...
        nextOrderId = trimmedOrderQuery.toUpperCase();
      }
    }
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (filters.cartNumber != null && filters.cartNumber > 0) count += 1;
  ...
  const renderHistoryRow = ({ item }: { item: HistoryRecord }) => {
    ...
    const displayOrderId = formatDisplayOrderNumber(item.cartNumber, item.orderId);
  ...
  const showListSkeleton = list.loading && !list.loadingMore;
    ...
    <>
      ...
      >
        <KeyboardAwareFlatList
          style={styles.list}
          data={showListSkeleton ? [] : list.items}
