...
export default function HistoryDetailsScreen({ navigation, route }: Props) {
  ...
  const showWarrantyModule = hasWarrantyModule(shop);
  const warrantyItemCount = useMemo(
    () => countWarrantyHistoryItems(record.items),
    [record.items],
  );
  const customerMobile = record.customerMobile.trim();
  const hasCustomerMobile = customerMobile.length > 0;
    ...
    async (customerMobile: string) => {
      ...
      try {
        ...
        show_Alert(
          'success',
          'SMS sent',
          'Customer receipt SMS has been sent with order and warranty details.',
          1,
          false,
          'OK',
          () => {},
  ...
  const handleConfirmSalesPersonUpdate = useCallback(async () => {
    ...
    try {
      ...
      show_Alert(
        ...
        `${updatedName ?? 'Sales person'} is now assigned to ${displayOrderId}.`,
        1,
        false,
        'OK',
        () => {
        ...
        >
              ...
              return (
                ...
                >
                  <View style={styles.itemBody}>
                    ...
                    {warrantyLine ? (
                      <Text style={[styles.itemWarranty, { color: paperTheme.colors.secondary }]}>
                        {warrantyLine}
                      </Text>
                    ) : null}
                  </View>
          ...
          >
            ...
            <Text style={[styles.sectionHint, { color: paperTheme.colors.onSurfaceVariant }]}>
              View the sales receipt or send an SMS with order details and a digital receipt link.
              {showWarrantyModule && warrantyItemCount > 0
                ? ' Warranty period and expiry are included for covered products.'
                : ''}
            </Text>

            <DetailRow
              label="SMS status"
              value={record.isSmsSent ? 'Sent to customer' : 'Not sent yet'}
              labelColor={paperTheme.colors.onSurfaceVariant}
              valueColor={
                record.isSmsSent ? paperTheme.colors.primary : paperTheme.colors.onSurfaceVariant
              }
            />
            ...
            {showWarrantyModule && warrantyItemCount > 0 ? (
              ...
              >
                ...
                />
                <Text style={[styles.warrantySummaryText, { color: paperTheme.colors.secondary }]}>
                  {warrantyItemCount} item{warrantyItemCount === 1 ? '' : 's'} with warranty on this
                  receipt
                </Text>
              </View>
            ) : null}
            ...
            <View style={styles.receiptActionsRow}>
              ...
              >
                ...
                <Text style={[styles.receiptActionText, { color: paperTheme.colors.primary }]}>
                  See receipt
                </Text>
              </TouchableOpacity>
              ...
              >
                {resendingBill ? (
                  ...
                  <>
                    ...
                    >
                      {record.isSmsSent ? 'Resend SMS' : 'Send SMS'}
                    </Text>
                  </>
                )}
              </TouchableOpacity>
            ...
            {!hasCustomerMobile ? (
              <Text style={[styles.sectionHint, { color: paperTheme.colors.onSurfaceVariant }]}>
                No customer phone saved on this sale. You can enter a number when sending the SMS.
              </Text>
            ) : null}
          </View>
        ...
        <View style={pickerStyles.overlay}>
          ...
          <View style={[pickerStyles.sheet, { backgroundColor: paperTheme.colors.surface }]}>
            ...
            <Text style={[pickerStyles.sub, { color: paperTheme.colors.onSurfaceVariant }]}>
              {hasAssignedSalesPerson
                ? `Choose who assisted this sale for ${displayOrderId}. The previous assignment will be replaced.`
                : `Choose a sales person for ${displayOrderId}. Only employees assigned to this branch are shown.`}
            </Text>
