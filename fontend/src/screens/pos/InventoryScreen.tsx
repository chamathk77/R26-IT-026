...
import { CartSessionSummary, CartOrderItem } from '../../type/cart';
...
export default function ProductsScreen({ navigation }: Props) {
  ...
  const [activeCartExpanded, setActiveCartExpanded] = useState(false);
  ...
  const toggleActiveCartExpanded = useCallback(() => {
    setActiveCartExpanded((prev) => {
      const next = !prev;
      if (
        next &&
        activeSession.sessionId &&
        (reviewSessionId !== activeSession.sessionId || !reviewOrder)
      ) {
        void dispatch(
          fetchCartItems_Service({
  ...
  }, [activeSession.sessionId, dispatch, reviewOrder, reviewSessionId]);

  const renderCartItemLine = useCallback(
    (item: CartOrderItem) => (
      <View key={getCartOrderItemKey(item)} style={styles.cartItemRow}>
        {item.productNumber ? (
          <View
            style={[
          ...
          >
            <Text
              style={[styles.cartItemNumberText, { color: paperTheme.colors.tertiary }]}
              numberOfLines={1}
            >
              {item.productNumber}
            </Text>
        ...
        <Text
          style={[styles.cartItemName, { color: paperTheme.colors.onSurface }]}
          numberOfLines={1}
        >
          {item.name}
        </Text>
        ...
        >
          <Text style={[styles.cartItemQtyText, { color: paperTheme.colors.primary }]}>
            {item.quantity}
          </Text>
        </View>
      </View>
        ...
        {activeSession.sessionId && activeSession.cartNumber ? (
          ...
          >
            <TouchableOpacity
              ...
              onPress={toggleActiveCartExpanded}
              activeOpacity={0.88}
              accessibilityRole="button"
              accessibilityLabel={
                activeCartExpanded ? 'Collapse active cart' : 'Expand active cart'
              }
            >
              <View
                style={[
              ...
              >
                <Ionicons
                  name={activeCartExpanded ? 'chevron-up' : 'chevron-down'}
                  size={18}
            ...
            {activeCartExpanded ? (
              <View
                style={[
                  styles.activeCartItemsBlock,
                  { borderTopColor: paperTheme.colors.outlineVariant },
                ]}
              >
                {reviewItemsLoading && reviewSessionId === activeSession.sessionId ? (
                  <ActivityIndicator
                    size="small"
                    color={paperTheme.colors.primary}
                    style={styles.activeCartLoading}
                  />
                ) : activeCartItems.length > 0 ? (
                  activeCartItems.map((item) => renderCartItemLine(item))
                ) : (
                  <Text
                    style={[styles.activeCartEmptyText, { color: paperTheme.colors.onSurfaceVariant }]}
                  >
                    Cart is empty — use + on products to add items
                  </Text>
                )}
              </View>
            ) : null}
                        ...
                        {isReviewing ? (
                          ...
                          >
                            {reviewItemsLoading ? (
                              ...
                              (reviewOrder?.items ?? []).map((item) => renderCartItemLine(item))
                            ) : (
                              <Text
                                style={[
                                  styles.activeCartEmptyText,
                                  { color: paperTheme.colors.onSurfaceVariant },
                                ]}
                              >
