...
import { paymentSubmit_Service } from '../../../../services/PaymentService';
...
import {
  ...
  takeReceiptPhoto,
} from '../upFrontPayment/paymentReceiptUpload';
...
type Props = NativeStackScreenProps<RootStackParamList, 'PendingPaymentBankTransfer'>;

function getPaymentTitle(paymentType: string, subscriptionType?: string | null): string {
  if (paymentType === 'upFront') return 'Up-front payment';
  if (paymentType === 'sms') return 'SMS package';
  if (subscriptionType === '1month') return 'Monthly plan';
  if (subscriptionType === '3months') return 'Quarterly plan';
  if (subscriptionType === '6months') return 'Half-year plan';
  if (subscriptionType === '1year') return 'Annual plan';
  return 'Subscription payment';
}

export default function PendingPaymentBankTransferScreen({ navigation, route }: Props) {
  ...
  const title = getPaymentTitle(payment.paymentType, payment.subscriptionType);
    ...
    try {
      ...
      setTimeout(() => {
        show_Alert(
          'error',
          'Submission failed',
          getApiErrorMessage(error, 'Could not submit payment. Please try again.'),
          1,
          false,
          'OK',
          () => {},
          ...
          >
            {receiptUri ? (
              ...
              >
                ...
                >
                  Take a clear photo of your payment receipt or choose one from your
                  gallery.
                </Text>
              </View>
            )}
