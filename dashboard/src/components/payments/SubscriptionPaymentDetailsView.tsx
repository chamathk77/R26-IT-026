...
import {
  ...
  isAdminReviewablePaymentStatus,
} from './paymentUi';
...
const SUBSCRIPTION_OVERDUE_THRESHOLD = 14;
const RESET_APPROVE_SHOP_STATUSES = new Set(['paymentPending', 'diactiveByAdmin']);

interface SubscriptionPaymentDetailsViewProps {
  paymentId: string;
}

export default function SubscriptionPaymentDetailsView({
  paymentId,
}: SubscriptionPaymentDetailsViewProps) {
  ...
  const canShowActions = isAdminReviewablePaymentStatus(payment?.status);
  const isOfflineNotPaid = payment?.status === 'notPaid';
  ...
  const isOverduePending =
    canShowActions && subscriptionDueDays > SUBSCRIPTION_OVERDUE_THRESHOLD;
  ...
  const handleApprove = async () => {
    ...
    try {
      const data = await approveSubscriptionPayment(paymentId);
      setPayment(data.payment);
      setShop(data.shop);
      setSuccessMessage(data.message || 'Subscription payment approved.');
    } catch (err) {
  ...
  const handleResetAndApprove = async () => {
    ...
    try {
      const data = await resetAndApproveSubscriptionPayment(paymentId);
      setPayment(data.payment);
      setShop(data.shop);
      setSuccessMessage(data.message || 'Subscription payment reset and approved.');
    } catch (err) {
      setError(getPaymentActionErrorMessage(err));
    } finally {
      setActionLoading(null);
  ...
  const handleReject = async () => {
    ...
    try {
      const data = await rejectSubscriptionPayment(paymentId, reason);
      setPayment(data.payment);
      setShop(data.shop);
      setSuccessMessage(data.message || 'Subscription payment rejected.');
      setRejectDialogOpen(false);
      setRejectReason('');
    } catch (err) {
  ...
  return (
    <>
      ...
      {isSubscriptionChangePending ? (
        ...
        >
          ...
          <Typography variant="body2">
            This shop requested a subscription plan change while on{' '}
            <strong>{formatSubscriptionTypeLabel(shop?.subscriptionType as string)}</strong>.
            After you approve this payment, the shop will be moved to{' '}
            <code>changeSubscription</code> status so the owner can select a new plan.
          </Typography>
        </Alert>
      ) : null}
      ...
      {isOverduePending ? (
        ...
        >
          <Typography variant="subtitle2" sx={{ fontWeight: 800, mb: 0.5 }}>
            Overdue subscription — settle required
          </Typography>
          <Typography variant="body2">
            This shop has <strong>{subscriptionDueDays}</strong> due days (over{' '}
            {SUBSCRIPTION_OVERDUE_THRESHOLD}). Prefer <strong>Reset and approve</strong> when the
            shop is <code>paymentPending</code> or <code>diactiveByAdmin</code> so billing restarts
            from today.
          </Typography>
        </Alert>
      ) : null}
          ...
          <Card sx={{ mb: 3, borderRadius: 3 }}>
            <CardContent sx={{ p: 3 }}>
              ...
              <Grid container spacing={2}>
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  <DetailField label="Subscription plan" value={formatSubscriptionTypeLabel(payment.subscriptionType)} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <DetailField
          ...
          <Card sx={{ borderRadius: 3 }}>
            <CardContent sx={{ p: 3 }}>
              ...
              <Grid container spacing={2}>
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  <DetailField label="Shop status" value={shop?.status} highlight={isOverduePending} />
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <DetailField
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                  <DetailField
                    ...
                    value={formatSubscriptionTypeLabel(shop?.subscriptionType as string)}
                  />
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <DetailField
                    label="Plan change requested"
                    value={isSubscriptionChangePending ? 'Yes' : 'No'}
                    highlight={isSubscriptionChangePending}
                  />
                </Grid>
              </Grid>
            </CardContent>
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              {isOfflineNotPaid ? (
                <Alert severity="info" sx={{ mb: 2, borderRadius: 2 }}>
                  No receipt was uploaded in the app. Confirm this payment only after verifying the
                  bank transfer offline (e.g. iOS user contacted support).
                </Alert>
              ) : null}
              ...
              {canShowActions ? (
                <Box sx={{ mt: 2, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                  ...
                  <Button
                    ...
                    variant="contained"
                    color="primary"
                    disabled={actionLoading !== null || !canResetAndApprove}
      ...
      <Dialog open={rejectDialogOpen} onClose={handleCloseRejectDialog} fullWidth maxWidth="sm">
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Provide a reason for rejecting this subscription payment. The shop owner will be
            notified.
          </Typography>
          <TextField
            autoFocus
