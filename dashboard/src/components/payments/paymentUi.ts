...
export function formatSubscriptionTypeLabel(type?: string | null): string {
  if (!type) return '—';
...
export const ADMIN_REVIEWABLE_PAYMENT_STATUSES = new Set(['pending', 'notPaid']);

export function isAdminReviewablePaymentStatus(status?: string | null): boolean {
  return status != null && ADMIN_REVIEWABLE_PAYMENT_STATUSES.has(status);
}
