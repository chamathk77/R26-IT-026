...
export default function PaymentDetailsView({
  ...
  const isUpfrontReviewable =
    showOnboardingActions &&
    isUpFrontPayment(payment?.paymentType) &&
    isAdminReviewablePaymentStatus(payment?.status);

  const isSubscriptionReviewable =
    showOnboardingActions &&
    isSubscriptionPayment(payment?.paymentType) &&
    isAdminReviewablePaymentStatus(payment?.status);

  const canShowActions = isUpfrontReviewable || isSubscriptionReviewable;
  const isOfflineNotPaid = payment?.status === 'notPaid';

  const rejectDialogTitle = isSubscriptionReviewable
    ? 'Reject subscription payment'
    : 'Reject upfront payment';

  const rejectDialogDescription = isSubscriptionReviewable
  ...
  const handleApprove = async () => {
    ...
    try {
      const data = isSubscriptionReviewable
