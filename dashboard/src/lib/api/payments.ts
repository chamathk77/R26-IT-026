...
import type {
  ...
  GenerateUpfrontConfirmationPayload,
  GenerateUpfrontConfirmationResponse,
  CreateManualPaymentConfirmationPayload,
  CreateManualPaymentConfirmationResponse,
  ManualPaymentConfirmationDetailsResponse,
  ManualPaymentConfirmationsResponse,
...
export async function generateUpfrontPaymentConfirmation(
  payload: GenerateUpfrontConfirmationPayload,
): Promise<GenerateUpfrontConfirmationResponse> {
  const response = await api.post<GenerateUpfrontConfirmationResponse>(
    '/api/dashboard/payments/generate-upfront-confirmation',
    payload,
  );
  return response.data;
}

export async function createManualPaymentConfirmation(
  payload: CreateManualPaymentConfirmationPayload,
): Promise<CreateManualPaymentConfirmationResponse> {
  const response = await api.post<CreateManualPaymentConfirmationResponse>(
    '/api/dashboard/manual-payment-confirmations',
    payload,
  );
  return response.data;
}

export async function fetchManualPaymentConfirmations(params?: {
  page?: number;
  limit?: number;
}): Promise<ManualPaymentConfirmationsResponse> {
  const response = await api.get<ManualPaymentConfirmationsResponse>(
    '/api/dashboard/manual-payment-confirmations',
    {
      params: {
        page: params?.page ?? 1,
...
export async function fetchManualPaymentConfirmation(
  confirmationId: string,
): Promise<ManualPaymentConfirmationDetailsResponse> {
  const response = await api.get<ManualPaymentConfirmationDetailsResponse>(
    `/api/dashboard/manual-payment-confirmations/${confirmationId}`,
  );
  return response.data;
}
